...
export default function BreakPage(){
  ...
  async function act(kind){
    setLoading(true)
    try{
      const idem = makeIdemKey('b')
      let geoFlag = null
      if (coords && store && typeof store.lat==='number'){
      ...
      if (res.ok){ alert((kind==='break_out'?'Break Out':'Break In') + (geoFlag?` · ${geoFlag}`:'')); setNote(''); return }
      await enqueue({ ...body }); alert('No network. Saved offline; will auto‑flush when online.')
    }catch(e){ await enqueue({ kind, note, coords, idem: makeIdemKey('b') }); alert('Saved offline; will auto‑flush when online.') }
    ...
    <div className="space-y-4">
      <div className="card space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button disabled={loading} className="btn btn-primary" onClick={()=>act('break_out')}>Start Break</button>
          <button disabled={loading} className="btn btn-primary" onClick={()=>act('break_in')}>End Break</button>
        </div>
